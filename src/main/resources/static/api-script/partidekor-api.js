...
var typename = path.split("/").pop().split(".")[0];
console.log( typename );


const url_ItemCode = 'http://localhost:8080/api/1.0/partidekorlar/';
const getAlldekorlar = 'http://localhost:8080/api/1.0/allpartidekorlar?type='+typename;
const page_url = `http://localhost:8080/api/1.0/partidekorlar?page=`;
...
searchInput.addEventListener("input", e => {
  console.log("xina dekorlar =", users);
...
var getAll = function (pageCount = 0) {
  ...
  .then(data => {
    ...
    users = data.map(user => {
      ...
      btn.innerHTML =  `       <form  action="../../magaza/standart/parti-item-details.html" class="thumb-link" method="get">
    ...
    .then(data => {
      ...
      <div class="product-inner tooltip-top tooltip-all-top">
          ...
          <div>
              ...
              <form  action="../magaza/standart/parti-item-details.html" class="thumb-link" method="get">

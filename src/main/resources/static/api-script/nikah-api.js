...
const getAlldekorlar = 'http://localhost:8080/api/1.0/allnikahdekorlar';
const page_url = `http://localhost:8080/api/1.0/nikahdekorlar?page=`;
...
searchInput.addEventListener("input", e => {
  console.log("dekorlar =", decors);

  const value = e.target.value.toUpperCase()
  decors.forEach(decor => {

    const isVisible =
      decor.itemCode.toUpperCase().includes(value)
      decor.element.classList.toggle("hide", !isVisible)
  })
})
...
var getAll = function (pageCount = 0) {
  ...
  .then(data => {
    ...
    decors = data.map(user => {
      ...
      btn.innerHTML =  `       <form  action="../magaza/standart/nikah-item-details.html" class="thumb-link" method="get">
